...
const Events = () => {
  const { userEvents }:any = useContext(AppContext)
  ...
  return (
    <div>
      ...
      <Table>
        <Table.Head>
          <Table.TextHeaderCell flexBasis={300} flexShrink={0} flexGrow={0}>Event</Table.TextHeaderCell>
        ...
        <Table.VirtualBody height={1000}>
          {userEvents.map((event) => (
            <Table.Row key={event.id} isSelectable onSelect={() => toaster.success(`Properties: ${JSON.stringify(event.properties)}`)}>
              ...
              <Table.TextCell flexBasis={300} flexShrink={0} flexGrow={0}>{event.timestamp}</Table.TextCell>
              <Table.TextCell flexBasis={800} >{JSON.stringify(event.properties)}</Table.TextCell>
            </Table.Row>
          ))}
        </Table.VirtualBody>
      </Table>
